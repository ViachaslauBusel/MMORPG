...
namespace OpenWorldEditor
{
    public class MachineLoader : MonoBehaviour
    {
        ...
        {
            Destroy();
            if (WindowSetting.MachineList.machines == null) return;
            foreach (Machine machine in WindowSetting.MachineList.machines)
            {
                if (Vector3.Distance(trackingObj.position, machine.position) < _areaVisible)
                {
                    GameObject prefabResource = machine.prefab;
                    CreatePrefab(prefabResource, machine.position, machine.rotation, machine.scale, 0.0f, machine.id);
                }
            }
        }
    }
