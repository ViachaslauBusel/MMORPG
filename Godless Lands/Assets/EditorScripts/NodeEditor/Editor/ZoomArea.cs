...
public class ZoomArea
{
    ...
    public void Begin()
    {
        ...
        NodeEditor.Grid.DrawGrid(new Vector2(m_size.x, m_size.y));
        m_instance = this;
    }
    /// <summary>
    ...
    private float CalculateEvent(float scale)
    {
        ...
        if (m_window.Contains(Event.current.mousePosition) && Event.current.type == EventType.MouseDown && Event.current.button == 1)
        {
